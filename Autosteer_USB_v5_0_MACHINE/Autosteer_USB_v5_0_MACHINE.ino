//dedicated version for lower and raise with D2 and D5 from pcb V2 of autosteer board for AgOpenGps V5.1.x or more 

//  always  test  with simulator   to understand
...
#define EEP_IdentMachine 0x5005
...
//machine
uint8_t AOGM[] = {0x80, 0x81, 0x7f, 0xED, 8, 0, 0, 0, 0, 0, 0, 0, 0, 0xCC };
uint8_t relayHiM = 0, relayLoM = 0, gpsSpeedM = 0, tramlineM = 0, treeM = 0, uTurnM = 0, hydLiftM = 0;
bool isRaise = false;
bool isLower = false;
...
//Variables for Machine config - 0 is false
struct Config {
  uint8_t raiseTime = 2;
  uint8_t lowerTime = 4;
  uint8_t enableToolLift = 0;
  uint8_t isMachineRelayActiveHigh = 1; //if zero, active low (default)
...
void setup()
{
  ...
  EEPROM.get(70, EEreadMachine);

  if (EEreadMachine != EEP_IdentMachine) // check on first start and write EEPROM
  {
    EEPROM.put(70, EEP_IdentMachine);
    EEPROM.put(76, aogConfig);
  }
  else
  {
    EEPROM.get(76, aogConfig);
  }
...
void loop()
{
  ...
  if (currentTime - lastTime >= LOOP_TIME)
  {
    ...
    {
      if (hydLiftM != lastTrigger && (hydLiftM == 1 || hydLiftM == 2))
      {
        lastTrigger = hydLiftM;
        lowerTimer = 0;
        raiseTimer = 0;
      ...
      if ((hydLiftM != 1 && hydLiftM != 2) || watchdogTimer >= WATCHDOG_FORCE_VALUE ) //|| gpsSpeed < 2)
      {
        lowerTimer = 0;
        raiseTimer = 0;
      ...
      if (aogConfig.isMachineRelayActiveHigh)
      {
  ...
  if (Serial.available() > dataLength && isHeaderFound && isPGNFound)
  {
    ...
    else if (pgn == 239) // EF Machine Data
    {
      ...
      hydLiftM = Serial.read();
      tramlineM = Serial.read();  //actual speed times 4, single uint8_t

      //just get the rest of bytes
      Serial.read();   //high,low bytes
      Serial.read();

      relayLoM = Serial.read();          // read relay control from AgOpenGPS
      relayHiM = Serial.read();

      if (aogConfig.isMachineRelayActiveHigh)
      {
        tramlineM = 255 - tramlineM;
        relayLoM = 255 - relayLoM;
      }
      ...
      AOGM[5] = aogConfig.isMachineRelayActiveHigh;
      AOGM[6] = (uint8_t)relayLoM;
      AOGM[7] = (uint8_t)tramlineM;


      //add the checksum
      int16_t CK_B = 0;
      for (uint8_t l = 2; l < sizeof(AOGM) - 1; l++)
      {
        CK_B = (CK_B + AOGM[l]);
      }
      AOGM[sizeof(AOGM) - 1] = CK_B;

      Serial.write(AOGM, sizeof(AOGM));
    ...
    else if (pgn == 238) //EE Machine Settings
    {
      ...
      if (bitRead(sett, 0)) aogConfig.isMachineRelayActiveHigh = 1; else aogConfig.isMachineRelayActiveHigh = 0;
